...
void ImpressionistUI::cb_useSobelFilter(Fl_Widget* o, void* v)
{
	ImpressionistUI *pUI = ((ImpressionistUI*)(o->user_data()));

	if (pUI->m_nUseSobelFilter == TRUE)
	{
		pUI->m_nUseSobelFilter = FALSE;
		pUI->m_BrushAngleSlider->activate();
	}
	else
	{
		pUI->m_nUseSobelFilter = TRUE;
		pUI->m_nUseBrushAngle = FALSE;
		pUI->m_UseBrushAngleButton->clear();
		pUI->m_BrushAngleSlider->deactivate();

	}
}

void ImpressionistUI::cb_useBrushAngle(Fl_Widget* o, void* v)
{
	ImpressionistUI *pUI = ((ImpressionistUI*)(o->user_data()));

	if (pUI->m_nUseBrushAngle == TRUE)
	{
		pUI->m_nUseBrushAngle = FALSE;
		pUI->m_BrushAngleSlider->activate();
	}
	...
	{
		pUI->m_nUseBrushAngle = TRUE;
		pUI->m_nUseSobelFilter = FALSE;
		pUI->m_UseSobelButton->clear();
		pUI->m_BrushAngleSlider->deactivate();

	}
}

void ImpressionistUI::cb_swapContents(Fl_Widget* o, void* v)
{
	ImpressionistUI *pUI = ((ImpressionistUI*)(o->user_data()));
	pUI->m_pDoc->swapContents();
}
...
//------------------------------------------------
// Return the brush angle in degrees
//------------------------------------------------
...
//-------------------------------------------------
// Set the brush angle in degrees
//-------------------------------------------------
...
//------------------------------------------------
// Returns true if the sobel filter should be used
//------------------------------------------------
bool ImpressionistUI::getSobel() 
{
	return m_nUseSobelFilter;
}

//------------------------------------------------
// Returns true if the brush angle should be used
//------------------------------------------------
bool ImpressionistUI::useBrushAngle() {
	return m_nUseBrushAngle;
}
...
Fl_Menu_Item ImpressionistUI::brushTypeMenu[NUM_BRUSH_TYPE+1] = {
  ...
  { "Triangle Brush", FL_ALT + 't', (Fl_Callback *)ImpressionistUI::cb_brushChoice, (void *)BRUSH_TRIANGLE },
  {0}
};
...
ImpressionistUI::ImpressionistUI() : m_nativeChooser(NULL) {
	...
	m_nAngle = 0;
	m_nAlpha = 1;
	m_nUseSobelFilter = false;
	m_nUseBrushAngle = false;
	...
	m_brushDialog = new Fl_Window(400, 325, "Brush Dialog");
		...
		m_BrushWidthSlider = new Fl_Value_Slider(10, 110, 300, 20, "Thickness");
		...
		m_UseSobelButton = new Fl_Light_Button(330, 220, 60, 20, "Sobel");
		m_UseSobelButton->user_data((void*)(this));   // record self to be used by static callback functions
		m_UseSobelButton->callback(cb_useSobelFilter);

		m_UseBrushAngleButton = new Fl_Light_Button(230, 220, 100, 20, "Brush Angle");
		m_UseBrushAngleButton->user_data((void*)(this));   // record self to be used by static callback functions
		m_UseBrushAngleButton->callback(cb_useBrushAngle);

		m_SwapContentsButton = new Fl_Button(10, 220, 100, 20, "Swap Contents");
		m_SwapContentsButton->user_data((void*)(this));   // record self to be used by static callback functions
		m_SwapContentsButton->callback(cb_swapContents);
...
void ImpressionistUI::normalizeFilter() {
	scale = 0;
	for (int i = 0; i < FLT_HEIGHT*FLT_WIDTH; i++) {
		scale += fltKernel[i];
	}
