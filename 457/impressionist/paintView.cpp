...
void PaintView::draw()
{
	...
	if ( m_pDoc->m_ucPainting && isAnEvent) 
	{
		...
		source.x = min(max(0, source.x), m_pDoc->m_nWidth - 1);
		source.y = min(max(0, source.y), m_pDoc->m_nHeight - 1);
		...
		case RIGHT_MOUSE_DOWN:
			m_pDoc->startRecordingRightClickDrag(target.x, target.y);
			break;
		case RIGHT_MOUSE_DRAG:

			break;
		case RIGHT_MOUSE_UP:
			m_pDoc->finishRecordingRightClickDrag(target.x, target.y);
			break;
