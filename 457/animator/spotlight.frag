...
void main()
{
	...
	vec3 spotL = vec3(gl_LightSource[2].position.xyz - v);
	
	//always add ambient
	ambient += gl_FrontLightProduct[2].ambient;

	float spotNdotL = max(dot(Normal, normalize(spotL)), 0.0);

	if (spotNdotL > 0.0) {
		float spotEffect = dot(normalize(spotL), normalize(gl_LightSource[2].spotDirection));
		if (spotEffect > gl_LightSource[2].spotCosCutoff) {
			spotEffect = pow(spotEffect, gl_LightSource[2].spotExponent);
			dist = length(spotL);
			float spotAttn = spotEffect / (gl_LightSource[2].constantAttenuation +
							  gl_LightSource[2].linearAttenuation * dist +
							  gl_LightSource[2].quadraticAttenuation * dist * dist);

			diffuse += spotAttn * gl_FrontLightProduct[2].diffuse * spotNdotL;

			vec3 spotHalfV = normalize(normalize(spotL) + Viewer);
			float spotNdotHalfV = max(dot(Normal, spotHalfV), 0.0);
			vec4 spotSpecular =  spotAttn * gl_FrontMaterial.specular * gl_LightSource[2].specular * pow(spotNdotHalfV, gl_FrontMaterial.shininess);
