...
SpotLight::SpotLight(const char* name, GLuint lightNumber,
	...
	float ar, float ag, float ab,
	float attA, float attB, float attC,
	float sx, float sy, float sz, float cut, float ang) :
	Light(name, lightNumber, x, y, z, dr, dg, db, sr, sg, sb, ar, ag, ab),
	attenA("Quadratic Attenuation ^ 5", 0, 1, attA, .05f),
	attenB("Linear Attenuation ^ 5", 0, 1, attB, .05f),
	attenC("Constant Attenuation ^ 5", 0, 1, attC, .05f),
	spotX("Spotlight X", -10, 10, sx, 0.05f),
	spotY("Spotlight Y", -10, 10, sy, 0.05f),
	spotZ("Spotlight Z", -10, 10, sz, 0.05f),
	cutoff("Cutoff", 0, 90, cut, 0.05f),
	angFalloff("Angular Falloff", 0, 10, ang, 0.05f)
{
	properties.add(&attenA).add(&attenB).add(&attenC).add(&spotX)
		.add(&spotY).add(&spotZ).add(&cutoff).add(&angFalloff);
}
...
void SpotLight::draw() {
	...
	glLightf(lightNumber, GL_SPOT_EXPONENT, angFalloff.getValue());
	GLfloat  spotDir[] = { spotX.getValue(), spotY.getValue(), spotZ.getValue() };
