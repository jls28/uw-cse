...
public:
	...
	RangeProperty attenA, attenB, attenC, spotX, spotY, spotZ, cutoff, angFalloff;
	...
	SpotLight(const char* name, GLuint lightNumber,
		...
		float sr = 1, float sg = 1, float sb = 1,
		float ar = .1, float ag = .1, float ab = .1,
		float attA = .4, float attB = .7, float attC = 0,
		float sx = 0, float sy = 1.0, float sz = 0.5, float cut = 30, float ang = 1);
