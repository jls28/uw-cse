...
//      terms = new ArrayList<RatTerm>(
//                Arrays.asList(new RatTerm(new RatNum(c), e))
//              );
      terms = new ArrayList<RatTerm>();
   ...
   */
  public int degree() {
    //this is probably going to fail some really edge case tests
  ...
  private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
    ...
    Comparator<RatTerm> expCompare = new Comparator<RatTerm>() {
      public int compare(RatTerm o1, RatTerm o2) {
        return o2.getExpt() - o1.getExpt();
      }
    };
    int index = Collections.binarySearch(lst, newTerm, expCompare);
    if (index < 0) {
      lst.add(-index-1, newTerm);
    } else {
      lst.get(index).add(newTerm);
    }
  }
  ...
  public RatPoly negate() {
    return new RatPoly(RatPoly.ZERO.sub(this).terms);
  }
  ...
  public RatPoly add(RatPoly p) {
    ...
    for (int i = maxDegree; i >= 0; i--) {
      RatTerm t_this = this.getTerm(i);
      RatTerm t_p = p.getTerm(i);
      RatTerm toAdd = t_this.add(t_p);
      if (!toAdd.isZero()) {
        sortedInsert(temp, toAdd);
      }
    }
  ...
  public RatPoly mul(RatPoly p) {
    ...
    for (int i = maxDegree; i >= 0; i--) {
      RatTerm t_this = this.getTerm(i);
      ArrayList<RatTerm> timeToFOIL = new ArrayList<RatTerm>();
      for (RatTerm t_p : p.terms) {
        sortedInsert(timeToFOIL, t_this.mul(t_p));
      }
      temp = temp.add(new RatPoly(timeToFOIL));
    }
    return temp;
  }
  ...
  public RatPoly div(RatPoly p) {
    ...
    RatPoly remainder = new RatPoly(this.terms);
    RatPoly denominator = new RatPoly(p.terms);

    while (!remainder.equals(ZERO) && remainder.degree() >= denominator.degree()) {
      RatPoly temp = new RatPoly(
              remainder.getTerm(remainder.degree())
              .div(denominator.getTerm(denominator.degree()))
      );
      quotient = quotient.add(temp);
      remainder = remainder.sub(temp.mul(denominator));
    }
    return quotient;
  }
  ...
  public RatPoly differentiate() {
    ArrayList<RatTerm> temp = new ArrayList<RatTerm>();
    for (RatTerm n : this.terms) {
      sortedInsert(temp, n.differentiate());
    }
    return new RatPoly(temp);
  }
  ...
  public RatPoly antiDifferentiate(RatNum integrationConstant) {
    ...
    ArrayList<RatTerm> temp = new ArrayList<RatTerm>();
    for (RatTerm n : this.terms) {
      sortedInsert(temp, n.antiDifferentiate());
    }
    sortedInsert(temp, new RatTerm(integrationConstant, 0));
    return new RatPoly(temp);
  }
