...
public final class RatPolyStack implements Iterable<RatPoly> {
  ...
  public RatPoly getNthFromTop(int index) {
    RatPoly[] temp = new RatPoly[this.polys.size()];
    this.polys.copyInto(temp);
    return temp[this.polys.size() - index - 1];
  }
  ...
  public void integrate() {
    this.push(
        this.pop().antiDifferentiate(new RatNum(0))
    );
  }
