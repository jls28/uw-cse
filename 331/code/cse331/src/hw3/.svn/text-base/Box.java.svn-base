...
public class Box implements Iterable<Ball> {
     ...
     * maxVolume is used to to keep track of the maximum amount of total volume a box can have
     */
    ...
    public boolean add(Ball b) {
        if (!ballContainer.contains(b) 
        	&& b.getVolume() + ballContainer.getVolume() <= this.maxVolume ) {
        	ballContainer.add(b);
        	return true;
        } else {
        	return false;
    ...
    public Iterator<Ball> getBallsFromSmallest() {
    	List<Ball> sorted = new ArrayList<Ball>();
    	Iterator<Ball> ballIterator = this.iterator();
    	while (ballIterator.hasNext()) {
    		sorted.add(ballIterator.next());
    	}
    	...
    	Collections.sort(sorted, volumeComparator);
