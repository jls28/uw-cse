...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer:
	/* Stefan: When I set the second argument to 11,
	nothing distinguishably different happened than 
	when I used 10. When I set it to 100, it printed
	'segmentation fault: 11' at the end, but it also
	still made the other arrays. When I set it to 
	1000, it didn't print 'done', and none of the other
	arrays were made, and it printed 'Segmentation
	fault' again, but with no number */
  ...
  // Answer:
  	// Stefan: Since fillArray is passed a pointer to an
  	// int named array, and not an 'actual' array, we can
  	// pass it a pointer to int, in this case the pointer
  	// to 'value', and it will change the int stored in
  	// 'value' to 2. 
  fillArray(&value, 1);
  ...
  // C++ programming.
  // Answer:
  	// Stefan: removing the free(heap_array) caused
  	// valgrind to report that 20 bytes of data 
  	// had been 'definitely lost', while replacing
  	// it caused valgrind to report that all heap
  	// blocks had been freed. 
  free(heap_array);
  ...
  // we used on the FourInts above.  Note the 
  // difference between FourInts and FourInts*.)
