...
void* mm_malloc (size_t size) {
  ...
  if (ptrFreeBlock == NULL) {
    ...
    //printf("shittingfuckfuck\n");
    ptrFreeBlock = searchFreeList(reqSize);
  }

  removeFreeBlock(ptrFreeBlock);
  size_t newSize = SIZE(ptrFreeBlock->sizeAndTags) - reqSize; //size of the potential new block
  size_t oldFreeBlockHeader = ptrFreeBlock->sizeAndTags; //saving the old block header
  size_t * ptrForSettingStuff = ptrFreeBlock;
  ptrFreeBlock->sizeAndTags |= TAG_USED;

  if (newSize >= MIN_BLOCK_SIZE) {
    //printf("gonna split shit\n");
    //set up the alloced block header area
    //theres enough space to fit a free block so split and make one
    //need to move to header of smaller free block and set it uuuuup.
    BlockInfo * splitFreeBlock = (BlockInfo *)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
    //header of alloced block
    ptrFreeBlock->sizeAndTags = reqSize | TAG_USED | (oldFreeBlockHeader & TAG_PRECEDING_USED);


    //can now basically make a blocksize and set it's fields

    //need to set its size, clear the alloc tag, and set the predecing used tag
    //BUG IS RIGHT HERE
    splitFreeBlock->sizeAndTags = newSize | TAG_PRECEDING_USED;

    //(*splitFreeBlock).sizeAndTags = ((SIZE(oldFreeBlockHeader) - reqSize) | TAG_PRECEDING_USED) & TAG_USED;

    //now set the footer
    size_t * footerPointer = UNSCALED_POINTER_ADD(splitFreeBlock, newSize - WORD_SIZE);
    *footerPointer = splitFreeBlock->sizeAndTags;
    //printf("%x", *footerPointer);
    //insert the new smaller free block to update the prev and next;
    insertFreeBlock(splitFreeBlock);
    //printf("done splitting\n");
  } else {
    //printf("oops im fat\n");
    (*ptrFreeBlock).sizeAndTags = oldFreeBlockHeader | TAG_USED;
    //fill up everything with the payload (so don't do anything really), go to the next block if it exists
    BlockInfo * nextBlock = (BlockInfo *)UNSCALED_POINTER_ADD(ptrFreeBlock, SIZE((*ptrFreeBlock).sizeAndTags));
    //set preceding use tag to 1 of the next block
    //oops, need to check to make sure we dont go outside the heap
    (*nextBlock).sizeAndTags |= TAG_PRECEDING_USED;

  }

  //return address of found block's "next" field.
  //examine_heap();
