...
void* mm_malloc (size_t size) {
  ...
  if (oldFreeBlockHeader - reqSize >= MIN_BLOCK_SIZE) {
    //enough for 4 words
...
void mm_free (void *ptr) {
  ...
  (*blockInfo).sizeAndTags = *ptrSave & ~1; //explodes the 'alloced' bit
  ...
  *ptrSave = (*blockInfo).sizeAndTags;
  if (ptrSave + 1 < mem_heap_hi()) {
    ...
    *ptrSave = (*ptrSave & ~2); //explodes the 'preceding block is alloced' bit
  }
