...
#define null NULL //cause lazy sometimes

void* mm_malloc (size_t size) {
  printf("starting to malloc poop\n");
  size_t reqSize;
  ...
  //so there's already a free block there, we dont need to worry about the null case
  //this is a lot simpler than i thought...

  //basic logic goes:
  //search for a free block of the min size.
  //if it exists, allocate it
  // if (searchFreeList(reqSize) != null) {
  //   //code for allocating a block
  // } else {
  //   //code for inserting a new block and allocating
  // }
  ...
  //we know the adress we want to return is the start of the payload.
  //which is the same as the freeBlock.next's address
  ptrToReturn = &((*ptrFreeBlock).next);

  //alloc space and return
  //set header to size (not sure what exactly), and the last bit to be allocated
  //need to go down to the free afterwards (if its there? and alloc)
  //oh shit and we need to preserve all that other shit ok
  //SIZE == SIZE OF WHOLE BLOCK INCLUDING HEADER AND FOOTER...

  //lets first save the shit we need from the free block so we can shrink it.
  BlockInfo * oldBlockToSave = ptrFreeBlock;
  size_t oldFreeBlockHeader = (*ptrFreeBlock).sizeAndTags;
  BlockInfo * oldFreeBlockNext = (*ptrFreeBlock).next;
  BlockInfo * oldFreeBlockPrev = (*ptrFreeBlock).prev;
  removeFreeBlock(oldBlockToSave);

  //now let's change the header of the alloced space to be the right thing
  (*ptrFreeBlock).sizeAndTags = (reqSize | TAG_USED);
  (*ptrFreeBlock).sizeAndTags = (*ptrFreeBlock).sizeAndTags | (oldFreeBlockHeader & TAG_PRECEDING_USED);

  //sets the last bit to show allocation
  //now need to set the reduced blocks header...
  //first should check if there is enough space somehow to fit a reduced size free block.
  printf("fuck?\n");


  if ((oldFreeBlockHeader & ~7) - reqSize >= MIN_BLOCK_SIZE) {
    //enough for 4 words
    //since there is enough space, make a pointer, move it down, and start setting shit.
    size_t * ptrForSettingStuff = &((*ptrFreeBlock).next); //going to start at the beginning of the payload
    ptrForSettingStuff = UNSCALED_POINTER_ADD(ptrForSettingStuff, (reqSize - WORD_SIZE));
    //now set the new blockinfo
    //make a blockinfo pointer here, set it up manually, and then add it back in. 

    size_t smallerFreeBlockHeader = (oldFreeBlockHeader - reqSize) | 2;
    printf("FUCKING setting stuff %x\n", ptrForSettingStuff);
    BlockInfo * newSmallerFreeBlock = ptrForSettingStuff;
    *ptrForSettingStuff = smallerFreeBlockHeader;
    printf("FUCKING setting stuff %x\n", ptrForSettingStuff);
    ptrForSettingStuff = UNSCALED_POINTER_ADD(ptrForSettingStuff, WORD_SIZE);
    printf("FUCKING setting stuff %x\n", ptrForSettingStuff);
    *ptrForSettingStuff = oldFreeBlockNext;
    printf("FUCKING setting stuff %x\n", ptrForSettingStuff);
    ptrForSettingStuff = UNSCALED_POINTER_ADD(ptrForSettingStuff, WORD_SIZE);
    printf("FUCKING setting stuff %x\n", ptrForSettingStuff);
    *ptrForSettingStuff = oldFreeBlockPrev;
    printf("FUCKING setting stuff %x\n", ptrForSettingStuff);
    //now set footer...
    //will need to go at least 8?
    ptrForSettingStuff = UNSCALED_POINTER_ADD(ptrForSettingStuff, (oldFreeBlockHeader - reqSize - 3 * WORD_SIZE)); 
    //^ lol math might be wrong but yolo
    *ptrForSettingStuff = smallerFreeBlockHeader;
    insertFreeBlock(newSmallerFreeBlock);
  } else {
    printf("fuck Im here!");
    //need to add remaining space to size.
    //since everythign is aligned it should be ok to do this
    (*ptrFreeBlock).sizeAndTags += (oldFreeBlockHeader - reqSize);
    //also need to remove the old block oops lol
  }
  return ptrToReturn; 
  //poopsmith
}
...
void mm_free (void *ptr) {
  ...
  size_t * ptrSave = (size_t *)ptr; //jic

  // Implement mm_free.  You can change or remove the declaraions
  // above.  They are included as minor hints.

  //insertFreeBlock(ptr); coalesceFreeBlock(ptr) will take care of prev and next and coalescing
  //so we just need to change the footer and header.
  //since we are supposedly given a pointer to a malloced block, we __KNOW it has a header
  ptrSave = UNSCALED_POINTER_SUB(ptrSave, WORD_SIZE);
  blockInfo = (BlockInfo *)ptrSave;
  //need to calc size of this freed block, should just be able to use header - wordsize
  //actually the old header should jsut be good enough.
  (*blockInfo).sizeAndTags = *ptrSave & ~TAG_USED; //explodes the 'alloced' bit
  //now need to move the pointer down to the footer.
  ptrSave = UNSCALED_POINTER_ADD(ptrSave, (((*blockInfo).sizeAndTags - WORD_SIZE) & ~7));
  //set the footer;
  *ptrSave = (*blockInfo).sizeAndTags;
  if (ptrSave + 1 < mem_heap_hi()) {
    ptrSave = UNSCALED_POINTER_ADD(ptrSave, WORD_SIZE);
    *ptrSave = (*ptrSave & ~TAG_PRECEDING_USED); //explodes the 'preceding block is alloced' bit
  }
  //and we should be all set, so insert and coalesce. 
  insertFreeBlock(blockInfo);
  coalesceFreeBlock(blockInfo);
  printf("fucking freed cracka!\n");
}
