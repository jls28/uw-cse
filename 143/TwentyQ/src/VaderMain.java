...
/**
 * The graphical user interface (GUI) that talks to your QuestionTree to play a
 * graphical 20 questions game ("The Sith Sense").
 */
public class VaderMain implements ActionListener, KeyListener, Runnable,
		UserInterface {
	private static final boolean CONSOLE_OUTPUT = true; // set to true to print
														// I/O messages
	...
	private static final String RESOURCE_URL = "http://www.cs.washington.edu/education/courses/cse143/13wi/homework/7/";
	...
	private static final boolean ITS_A_TRAP = true;
	...
	public VaderMain() {
		...
		if (ensureFile(BACKGROUND_IMAGE_FILE_NAME)) {
			// vader.setIcon(new
			// ImageIcon(ClassLoader.getSystemResource(BACKGROUND_IMAGE_FILE_NAME)));
		...
		setupComponent(messageLabel, new Point(20, 120),
				new Dimension(365, 125));
		...
		setupComponent(bannerLabel, new Point(0, 0),
				new Dimension(vader.getWidth(), 30));
		...
		yesButton = makeButton(YES_MESSAGE, new Point(340, 50), new Dimension(
				80, 30));
		noButton = makeButton(NO_MESSAGE, new Point(440, 50), new Dimension(80,
				30));
	...
	public void keyPressed(KeyEvent event) {
		if (!yesButton.isVisible() || event.isAltDown()
				|| event.isControlDown()) {
			return;
		}
	...
	private void doEnabling() {
		...
		statsArea.setText(String.format(STATUS_MESSAGE, game.totalGames(),
				game.gamesWon()));
	}
		...
		if (musicBox.isSelected()) {
			try {
				if (musicClip == null) {
					...
					if (Math.random() < 0.66) {
						rand = 1; // 2/3 chance of picking default empire theme
									// song
					}
