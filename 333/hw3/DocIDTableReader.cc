...
bool DocIDTableReader::LookupDocID(const DocID_t &docid,
  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    // MISSING:
    if (fseek(file_, next_offset, SEEK_SET) != 0) {
      return false;
    }
    ...
    if (header.docid == docid) {
      ...
      // MISSING:
      for (uint64_t i = 0; i < header.num_positions; i++) {
        docid_element_position elementPosition;
        // should already be seeked to the correct offset
        Verify333(fread(&elementPosition, sizeof(elementPosition), 1, file_) ==
                  1);
        elementPosition.toHostFormat();
        // Return the positions list through the output parameter,
        ret_list->push_back(elementPosition.position);
      }
      // and return true
      return true;
    }
...
list<docid_element_header> DocIDTableReader::GetDocIDList() {
  ...
  for (HWSize_t i = 0; i < header_.num_buckets; i++) {
    ...
    bucket_rec b_rec;
    // MISSING:
    fread(&b_rec, sizeof(b_rec), 1, file_);
    b_rec.toHostFormat();
    ...
    for (HWSize_t j = 0; j < b_rec.chain_len; j++) {
      ...
      Verify333(fseek(file_,
                      b_rec.bucket_position + j * sizeof(element_position_rec),
                      SEEK_SET) == 0);

      // Read the next element position from the bucket header.
      element_position_rec ep;
      // MISSING:
      fread(&ep, sizeof(ep), 1, file_);
      ep.toHostFormat();
