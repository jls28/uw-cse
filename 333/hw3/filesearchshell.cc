...
int main(int argc, char **argv) {
  ...
  // create a query processor, load all the given idx
  list<string> indexLocations;
  for (int i = 1; i < argc; i++) {
    indexLocations.push_back(argv[i]);
  }
  std::unique_ptr<hw3::QueryProcessor> queryProcessor(
      new hw3::QueryProcessor(indexLocations, false));
  // get input, create string array
  while (1) {
    ...
    // ProcessQuery on array
    auto begin = std::chrono::high_resolution_clock::now();
    std::vector<hw3::QueryProcessor::QueryResult> results =
        queryProcessor->ProcessQuery(tokens);
    auto end = std::chrono::high_resolution_clock::now();
    auto dur = end - begin;
    auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
    // format the resulting list nicely
    if (results.empty()) {
      std::cout << "  [no results]" << std::endl;
    }
    while (!results.empty()) {
      hw3::QueryProcessor::QueryResult temp = results.front();
      std::cout << "  " << temp.document_name << " (" << temp.rank << ")"
                << std::endl;
