...
typedef struct bucket_rec_st {
  ...
  void toDiskFormat() {
    chain_len = htonl(chain_len);
    bucket_position = htonl(bucket_position);
  }
  void toHostFormat() {
...
typedef struct doctable_element_header_st {
  ...
  void toDiskFormat() {
    docid = htonll(docid);
    filename_len = htons(filename_len);
  }
  void toHostFormat() {
    docid = ntohll(docid);
...
typedef struct worddocset_header_st {
  ...
  void toDiskFormat() {
    word_len = htons(word_len);
    doctable_len = htonl(doctable_len);
  }
  void toHostFormat() {
    word_len = ntohs(word_len);
    doctable_len = ntohl(doctable_len);
  }
} worddocset_header;
