...
FileIndexReader::FileIndexReader(std::string filename, bool validate) {
  ...
  Verify333((HWSize_t)f_stat.st_size ==
            (HWSize_t)(sizeof(IndexFileHeader) + header_.doctable_size +
                       header_.index_size));
  ...
  if (validate) {
    ...
    fseek(file_, sizeof(IndexFileHeader), SEEK_SET);
    while (left_to_read > 0) {
      // MISSING:
      uint8_t buf[] = {0};
      fread(buf, sizeof(uint8_t), 1, file_);
