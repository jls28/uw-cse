...
void FreeMemIndex(MemIndex index) { FreeHashTable(index, &MIFree); }
...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    InsertHashTable(index, kv, &oldkv);
  } else {
  ...
  hitkv.key = (HTKey_t)docid;
  hitkv.value = (HTValue_t)positions;

  res = InsertHashTable(wds->docIDs, hitkv, &oldkv);

  return 1;
}

void FreePayload(LLPayload_t *n) { free(n); }

LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  LinkedList retlist;
  HTKeyValue kv, docIDkv;
  WordDocSet *wds;
  int res;
  HTKey_t wordkey;
  ...
  if (res == 1) {
    wds = (WordDocSet *)kv.value;
    HTIter docIdIterator = HashTableMakeIterator(wds->docIDs);
    do {
      HTIteratorGet(docIdIterator, &docIDkv);
      DocID_t id = (DocID_t)docIDkv.key;
      LinkedList *documents = (LinkedList *)docIDkv.value;
      SearchResult *result;
      result = malloc(sizeof(*result));
      Verify333(result != NULL);
      result->docid = id;
      result->rank = NumElementsInLinkedList((LinkedList)documents);
      PushLinkedList(retlist, (LLPayload_t *)result);
    } while (HTIteratorNext(docIdIterator));
    HTIteratorFree(docIdIterator);
  } else {
    FreeLinkedList(retlist, (LLPayloadFreeFnPtr)&FreePayload);
    return NULL;
  }
  ...
  for (i = 1; i < qlen; i++) {
    ...
    for (j = 0; j < ne; j++) {
      ...
      SearchResult *curResult = ((SearchResult *)payload);
      wds = (WordDocSet *)kv.value;
      res = LookupHashTable(wds->docIDs, (HTKey_t)curResult->docid, &docIDkv);
      if (res == 1) {
        curResult->rank += NumElementsInLinkedList(docIDkv.value);
        LLIteratorNext(llit);
      } else {
        LLIteratorDelete(llit, (LLPayloadFreeFnPtr)&FreePayload);
      }
    }
    LLIteratorFree(llit);
  }
