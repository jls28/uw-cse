...
char *ReadFile(const char *filename, HWSize_t *size) {
  ...
  readBuffer = malloc(filestat.st_size + 1);
  if (readBuffer == NULL) {
    return NULL;
  }
  ...
  while (left_to_read > 0) {
    char singleChar[1];
    result = read(fd, singleChar, 1);
    if (result >= 0) {
      readBuffer[filestat.st_size - left_to_read] = singleChar[0];
      left_to_read--;
    } else {
      return NULL;
    }
...
static void LoopAndInsert(HashTable tab, char *content) {
  ...
  while (size > 0) {
    ...
    int curChar = *curptr;
    // printf("counter: %d\n", counter);
    bool isAlpha = isalpha(curChar);
    // if char is alpha and wordstart is null, set wordstart
    if (isAlpha) {
      *curptr = tolower(*curptr);
      if (wordstart == NULL) {
        wordstart = curptr;
      }
      // if char is alpha and wordstart is not null, continue
      else if (wordstart != NULL) {
        // continue
      }
    } else {
      // if char is not alpha, set to \0
      *curptr = '\0';
      if (wordstart == NULL) {
        // continue
      }
      // if char is not alpha and wordstat is not null, add to hash table
      else if (wordstart != NULL) {
...
static void AddToHashtable(HashTable tab, char *word, DocPositionOffset_t pos) {
  ...
  if (retval == 1) {
    ...
    Verify333(wp != NULL);
    newstr = malloc(strlen(word) + 1);  // Need to include '\0'
    Verify333(newstr != NULL);
    strcpy(newstr, word);
    wp->word = newstr;
    wp->positions = AllocateLinkedList();
    Verify333(wp->positions != NULL);
    retval = PushLinkedList(wp->positions, (LLPayload_t)((intptr_t)pos));
    Verify333(retval != 0);
    kv.key = hashKey;
