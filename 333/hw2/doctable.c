...
void DummyFree(HTValue_t payload) { free(payload); }

void FreeDocTable(DocTable table) {
  Verify333(table != NULL);

  // STEP 1.
  FreeHashTable(table->docid_to_docname, &DummyFree);
  FreeHashTable(table->docname_to_docid, &DummyFree);
  free(table);
}
...
DocID_t DTRegisterDocumentName(DocTable table, char *docname) {
  ...
  if (LookupHashTable(
          table->docname_to_docid,
          (HTKey_t)FNVHash64((unsigned char *)docname, strlen(docname)),
          &oldkv) == 1) {
    free(doccopy);
    free(docid);
...
DocID_t DTLookupDocumentName(DocTable table, char *docname) {
  ...
  res = LookupHashTable(
      table->docname_to_docid,
      (HTKey_t)FNVHash64((unsigned char *)docname, strlen(docname)), &kv);
  if (res != 1) {
    return 0;
  }
