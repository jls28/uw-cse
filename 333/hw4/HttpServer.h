...
class HttpServer {
 public:
  ...
  explicit HttpServer(uint16_t port, std::string staticfile_dirpath,
                      std::list<std::string> indices)
      : ss_(port),
        staticfile_dirpath_(staticfile_dirpath),
        indices_(indices),
        kNumThreads(100) {}
...
class HttpServerTask : public ThreadPool::Task {
 public:
  explicit HttpServerTask(ThreadPool::thread_task_fn f) : ThreadPool::Task(f) {}
