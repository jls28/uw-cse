...
void GetPortAndPath(int argc, char **argv, uint16_t *port, std::string *path,
...
void GetPortAndPath(int argc, char **argv, uint16_t *port, std::string *path,
  ...
  int tempPort = std::stoi(argv[1]);
  if (tempPort <= 65535 && tempPort >= 0) {
    *port = (uint16_t)tempPort;
  }

  // path is a readable directory
  if (opendir(argv[2]) == nullptr) {
    // Can't open dir
    Usage(argv[0]);
  }
  *path = argv[2];
  ...
  for (int i = 3; i < argc; i++) {
    // look at each index, make sure each is readable
    std::ifstream tempIndex(argv[i]);
    if (tempIndex.good()) {
