...
bool IsPathSafe(string rootdir, string testfile) {
  ...
  if (!realpath(rootdir.c_str(), rootpathbuffer)) return false;
  if (!realpath(testfile.c_str(), testfilebuffer)) return false;

  // Make sure that testfile is a prefix of rootdir.
  if (strlen(testfilebuffer) <= strlen(rootpathbuffer)) return false;
...
string URLParser::get_path() const { return path_; }

map<string, string> URLParser::get_args() const { return args_; }
...
bool ConnectToServer(string hostname, uint16_t portnum, int *client_fd) {
  ...
  // Do the lookup.
  if ((retval = getaddrinfo(hostname.c_str(), portstr, &hints, &results)) !=
      0) {
