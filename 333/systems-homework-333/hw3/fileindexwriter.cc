...
static HWSize_t WriteHeader(FILE *f, HWSize_t doct_size, HWSize_t memidx_size);
...
typedef HWSize_t (*write_element_fn)(FILE *f, IndexFileOffset_t offset,
                                     HTKeyValue *kv);
...
static HWSize_t WriteHashTable(FILE *f, HashTable t, IndexFileOffset_t offset,
                               write_element_fn fn);
...
static HWSize_t WriteBucketRecord(FILE *f, LinkedList li,
...
static HWSize_t WriteBucket(FILE *f, LinkedList li, IndexFileOffset_t offset,
                            write_element_fn fn);
...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  // MISSING:
  if (WriteHeader(f, dtres, mires) == 0) {
    fclose(f);
    unlink(filename);
    return 0;
  }
...
static HWSize_t WriteDocidDocnameFn(FILE *f, IndexFileOffset_t offset,
                                    HTKeyValue *kv) {
  size_t res;
  uint16_t slen_ho;
...
static HWSize_t WriteDocTable(FILE *f, DocTable dt, IndexFileOffset_t offset) {
  ...
  return WriteHashTable(f, DTGetDocidTable(dt), offset, &WriteDocidDocnameFn);
}
...
// offset 'offset'.
static HWSize_t WriteDocPositionListFn(FILE *f, IndexFileOffset_t offset,
                                       HTKeyValue *kv) {
  size_t res;
  ...
  docid_element_header head = {docID_ho, num_pos_ho};
  head.toDiskFormat();
  Verify333(fwrite(&head, sizeof(head), 1, f) == 1);
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    // MISSING:
    LLPayload_t temp;
    LLIteratorGetPayload(it, &temp);
    position.position = (uint64_t)temp;
  ...
  return sizeof(head) + num_pos_ho * sizeof(DocPositionOffset_t);
}

// This write_element_fn is used to write a WordDocSet
// element into the file at position 'offset'.
static HWSize_t WriteWordDocSetFn(FILE *f, IndexFileOffset_t offset,
                                  HTKeyValue *kv) {
  ...
  HWSize_t htlen_ho = WriteHashTable(
      f, wds->docIDs, offset + sizeof(worddocset_header) + wordlen_ho,
  ...
  Verify333(fwrite(&header, sizeof(header), 1, f) == 1);

  // Write the word itself, excluding the nullptr terminator,
  // in the right place in the file.
  // MISSING:
  Verify333(fwrite(wds->word, wordlen_ho, 1, f) == 1);
...
static HWSize_t WriteMemIndex(FILE *f, MemIndex mi, IndexFileOffset_t offset) {
  ...
  // final argument.
  return WriteHashTable(f, mi, offset, &WriteWordDocSetFn);
}
...
static HWSize_t WriteBucketRecord(FILE *f, LinkedList li,
  ...
  // MISSING:
  Verify333(fwrite(&br, sizeof(br), 1, f) == 1);
...
static HWSize_t WriteBucket(FILE *f, LinkedList li, IndexFileOffset_t offset,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      HTKeyValue *kv;
      // error is in here
      // looks like I need to write all the positions first and then the
      // elements
      // (themselves composed of positions)
      // MISSING:
      // need to write nextelpos
      if (fseek(f, offset + (sizeof(element_position_rec) * j), SEEK_SET) !=
          0) {
        return 0;
      }
      element_position_rec temp = {nextelpos};
      temp.toDiskFormat();
      Verify333(fwrite(&temp, sizeof(uint32_t), 1, f) == 1);
...
static HWSize_t WriteHashTable(FILE *f, HashTable t, IndexFileOffset_t offset,
  ...
  next_bucket_offset =
      next_bucket_rec_offset + (ht->num_buckets) * sizeof(bucket_rec);
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    // MISSING:
    next_bucket_rec_offset += WriteBucketRecord(
        f, ht->buckets[i], next_bucket_rec_offset, next_bucket_offset);
    if (NumElementsInLinkedList(ht->buckets[i]) != 0) {
      next_bucket_offset +=
          WriteBucket(f, ht->buckets[i], next_bucket_offset, fn);
    }
  }
