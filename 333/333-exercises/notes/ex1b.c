...
void CopyAndSort(uint64_t *source, uint64_t *dest, uint64_t len);
...
// Print out the contents of array with len elements to stdout.
void PrintArray(uint64_t *array, uint64_t len);
...
void PrintArray(uint64_t *array, uint64_t len) {
  for (uint64_t i = 0; i < len; i++) {
    printf("%" PRIu64 "%c", array[i], i == (len - 1) ? '\n' : ' ');
  }
}
