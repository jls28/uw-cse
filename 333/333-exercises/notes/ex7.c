...
/* copy all files in a given directory with names ending in ".txt" to stdout */
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    if ((namlen >= 4) && (strcmp(next->d_name + (namlen - 4), ".txt") == 0)) {
...
void ReadAndPrint(char *directory, char *file) {
  ...
  char *merged_filename = malloc(dlen + flen + 2);
